...
public class SettlementClient(NetWorkType network, string privateKey, string shopId) : Client(network)
{
    ...
    public async Task<string[]> GetSettlementClientList(int startIndex, int endIndex)
    {
        var response =
            await GetAsync(
                $"{RelayEndpoint}/v1/shop/settlement/client/list/{ShopId}?startIndex={startIndex}&endIndex={endIndex}&endIndex={endIndex}");
        var jObject = ParseResponseToJObject(response);
        ...
        for (var i = 0; i < clients.Count; i++) clientList[i] = clients[i].ToString();
        return clientList;
    }
    ...
    {
        var message =
            CommonUtils.GetCollectSettlementAmountMultiClientMessage(ShopId, clients, await GetShopNonceOf(Address),
                await GetChainId());
    ...
    public async Task<string> Refund(BigInteger amount)
    {
        ...
        var message =
            CommonUtils.GetShopRefundMessage(ShopId, adjustedAmount, await GetShopNonceOf(Address), await GetChainId());
        var signature = CommonUtils.SignMessage(KeyPair, message);
